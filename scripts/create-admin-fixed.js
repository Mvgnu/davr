...
async function createAdminUser() {
  try {
    ...
    // First check if we have a password_hash column
    try {
      ...
      // Insert the admin user with the correct column name
      let result;
      if (columnCheck.rows.length > 0) {
        // Use password_hash column
        result = await client.query(
          'INSERT INTO users (name, email, password_hash, role, created_at, updated_at) VALUES ($1, $2, $3, $4, NOW(), NOW()) RETURNING id',
          [name, email, hashedPassword, 'admin']
        );
        console.log('Used password_hash column for authentication');
      } else {
        // Use password column and add a warning
        result = await client.query(
          'INSERT INTO users (name, email, password, role, created_at, updated_at) VALUES ($1, $2, $3, $4, NOW(), NOW()) RETURNING id',
          [name, email, hashedPassword, 'admin']
        );
        console.log('⚠️ Warning: Using password column but authentication system expects password_hash');
        console.log('⚠️ Authentication might fail. Consider adding a password_hash column.');
      }
