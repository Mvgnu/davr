...
interface ClaimRecyclingCenterFormProps {
...
export default function ClaimRecyclingCenterForm({
  recyclingCenterId,
  recyclingCenterName,
  onSuccess,
  onClose,
}: ClaimRecyclingCenterFormProps) {
  ...
  async function onSubmit(data: ClaimFormValues) {
    ...
    try {
      ...
      if (response.data.success) {
        toast.success(response.data.message || 'Ihre Anfrage wurde erfolgreich eingereicht');
    ...
    return (
      <Card>
        ...
        <CardContent className="space-y-4">
          <p>
            Wir haben Ihre Anfrage zur Übernahme dieses Recycling-Centers erhalten. Unser Administrationsteam wird Ihre Anfrage prüfen und bearbeiten.
          </p>
          <p>
            Sie werden per E-Mail benachrichtigt, sobald Ihre Anfrage genehmigt oder abgelehnt wurde. Dies kann einige Werktage dauern.
          </p>
          <p className="text-sm text-gray-500">
            Bei Fragen zu Ihrer Anfrage kontaktieren Sie bitte unseren Support.
          </p>
        </CardContent>
        <CardFooter>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <CardContent className="space-y-4">
            ...
            <FormField
              ...
              name="name"
              render={({ field }: { field: ControllerRenderProps<ClaimFormValues, "name"> }) => (
                <FormItem>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField
                control={form.control}
                name="email"
                render={({ field }: { field: ControllerRenderProps<ClaimFormValues, "email"> }) => (
                  <FormItem>
              ...
              <FormField
                control={form.control}
                name="phone"
                render={({ field }: { field: ControllerRenderProps<ClaimFormValues, "phone"> }) => (
                  <FormItem>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField
                control={form.control}
                name="companyName"
                render={({ field }: { field: ControllerRenderProps<ClaimFormValues, "companyName"> }) => (
                  <FormItem>
              ...
              <FormField
                control={form.control}
                name="businessRole"
                render={({ field }: { field: ControllerRenderProps<ClaimFormValues, "businessRole"> }) => (
                  <FormItem>
            ...
            <FormField
              control={form.control}
              name="message"
              render={({ field }: { field: ControllerRenderProps<ClaimFormValues, "message"> }) => (
                <FormItem>
