...
export default function CitySearch({ initialCities, popularCities }: CitySearchProps) {
      ...
      <div className="mb-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {popularCities.slice(0, 10).map(city => (
            <Link 
              key={city.name} 
              href={`/recycling-centers?city=${encodeURIComponent(city.name)}`}
            ...
            <div key={letter} className="mb-8">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {alphabeticalGroups[letter].map(city => (
                  <Link 
                    key={city} 
                    href={`/recycling-centers?city=${encodeURIComponent(city)}`}
